...
type Storage interface {
	...
	SaveEvents(ctx context.Context, events []tvcalendar.Event) error

	// GetEvents retrieves events falling within the specified date range.
	GetEvents(ctx context.Context, start, end time.Time) ([]tvcalendar.Event, error)

	// GetEventsByCurrency retrieves events matching a specific currency code.
	GetEventsByCurrency(ctx context.Context, currency string) ([]tvcalendar.Event, error)

	// QueryEvents retrieves events matching a set of dynamic filter criteria.
	QueryEvents(ctx context.Context, filter QueryFilter) ([]tvcalendar.Event, error)
